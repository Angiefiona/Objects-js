// Given an array of objects, each object representing a person with a name and age property, write a function 
// that returns a new array containing the names of all people who are 18 years old or older.
const people = [
  { name: 'Alice', age: 17 },
  { name: 'Eunice', age: 22 },
  { name: 'Charlie', age: 14 },
...
function studentScore(score){
  const averageScores = [];
    for(let i = 0; i<score.length; i++){
        const average = score[i];
        if(average.scores>=85){
           averageScores.push(average.name);
        }
    }
    return averageScores;
}
const averageScores = studentScore(score)
